...
export default class IsoSprite extends Sprite {
	...
	resetIsoBounds() {
		...
		this._isoBounds.x =
			this.isoX + this._isoBounds.widthX * -this.originX + this._isoBounds.widthX * 0.5;
		this._isoBounds.y =
			this.isoY + this._isoBounds.widthY * this.originX - this._isoBounds.widthY * 0.5;
		this._isoBounds.z =
			this.isoZ - Math.abs(this.height) * (1 - this.originY) + Math.abs(this.width * 0.5);
